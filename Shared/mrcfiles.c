                    ...
                    char axis)
{
  ...
  rewind(fout);
  j = fseek(fout, hdata->headerSize, SEEK_SET);
  if (j) {
    b3dError(stderr, "ERROR: mrc_write_slice - on initial seek (returned %d)\n", j);
    return(-1);
  }
  data = (unsigned char *)buf;
  nx = hdata->nx;
  ...
  case 'Z':
   ...
   j = mrcHugeSeek(fout, 0, 0, 0, slice, nx, ny, dcsize, SEEK_CUR);
  if (j) {
    b3dError(stderr, "ERROR: mrc_write_slice - on huge seek (returned %d)\n", j);
    return(-1);
  }
